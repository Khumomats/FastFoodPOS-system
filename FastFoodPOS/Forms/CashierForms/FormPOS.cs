...
namespace FastFoodPOS.Forms.CashierForms
{
    partial class FormPOS : BaseForm
    {
...

        {
            ...
            {
                if (rbtCard.Checked == false)

                {
                    change = TextPayment.Value - Total;
                    if (change < 0) throw new Level1Exception("Your payment is " + Math.Abs(change) + "short");
                }
            }
            return change;
        }
        ...
        private void ButtonCalculateChange_Click(object sender, EventArgs e)
        {
            try
            {
                LabelChange.Text = "R"+GetChange().ToString() ;
            }
